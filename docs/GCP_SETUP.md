...
- Access to the GCP project (e.g., `rle-scamacho`)
...
```bash
gcloud projects add-iam-policy-binding "rle-scamacho" \
  --member="serviceAccount:github-actions-rle@rle-scamacho.iam.gserviceaccount.com" \
...
```bash
gcloud projects describe rle-scamacho --format="value(projectNumber)"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  "github-actions-rle@rle-scamacho.iam.gserviceaccount.com" \
  --project="rle-scamacho" \
...
| `GCP_SERVICE_ACCOUNT` | `github-actions-rle@rle-scamacho.iam.gserviceaccount.com` |
...
```bash
python scripts/rasterize_ecosystem_to_cog.py --project rle-scamacho
```
...
- **enables the `storage.googleapis.com` API** and **creates a public-read bucket** `gs://rle-scamacho-rle-cogs` if it does not already exist;
...
```
Caller does not have required permission to use project rle-scamacho.
